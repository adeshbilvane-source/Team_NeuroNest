...
export default function DoctorChatListPage() {
  ...
  return <div className="min-h-screen bg-canvas font-ui flex flex-col">
    ...
    <div className="flex-1 overflow-y-auto px-4.5 py-4.5 pb-8"><div className="bg-white rounded-2xl px-3.5 py-2.75 flex items-center gap-2.5 shadow-sm mb-4"><Search size={18} className="text-ink-soft flex-shrink-0" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations..." className="w-full border-0 outline-none bg-transparent font-ui text-[13px] font-bold text-ink" /></div>
      {results.map((conversation) => <button key={conversation.id} onClick={() => navigate(`/doctor/chat/${conversation.id}`)} className={`w-full bg-white rounded-[18px] px-3.5 py-3 mb-2.5 flex items-center gap-3 shadow-sm text-left ${conversation.urgent ? 'border-[1.5px] border-alert-red' : ''}`}><PatientAvatar patientId={conversation.id} initials={conversation.initials} name={conversation.name} className={`w-11 h-11 rounded-full ${conversation.urgent ? 'bg-alert-red' : 'bg-[#9AA69C]'} text-white flex items-center justify-center font-extrabold text-sm flex-shrink-0`} /><div className="min-w-0 flex-1"><h2 className="m-0 mb-0.5 text-[14.5px] text-ink font-extrabold flex items-center gap-1.5 truncate">{conversation.name}{conversation.urgent && <span className="text-[9px] font-black bg-red-tint text-alert-red px-1.75 py-0.5 rounded-full">Urgent</span>}</h2><p className="m-0 text-[11.5px] text-ink-soft font-bold truncate">{conversation.message}</p></div><div className="text-right flex-shrink-0"><div className="text-[10.5px] text-ink-soft font-bold">{conversation.time}</div>{conversation.unread && <div className="w-2.25 h-2.25 rounded-full bg-alert-red mt-1.25 ml-auto" />}</div><ChevronRight size={15} className="text-ink-soft flex-shrink-0" /></button>)}
